...
import type { Politician, ApiResponse, UpdatePoliticianRequest } from '~/types'
...
const politicianId = route.params.id as string
...
const form = reactive<UpdatePoliticianRequest>({
  name: '',
  slug: '',
  photo: '',
  position: '',
...
async function loadPolitician() {
  loading.value = true
  try {
    const response = await $fetch<ApiResponse<Politician>>(`${baseUrl}/admin/politicians/${politicianId}`, {
      headers: auth.getAuthHeaders()
    })

    if (response.success) {
      const politician = response.data
      form.name = politician.name
      form.slug = politician.slug
      form.photo = politician.photo || ''
      form.position = politician.position || ''
      form.party = politician.party || ''
      form.short_bio = politician.short_bio || ''
    }
  } catch (e: unknown) {
    const err = e as { data?: { error?: { message?: string } } }
    error.value = err?.data?.error?.message || 'Failed to load politician'
  }
  loading.value = false
}
...
async function handleSubmit() {
  ...
  try {
    const payload: UpdatePoliticianRequest = {
    ...
    await $fetch(`${baseUrl}/admin/politicians/${politicianId}`, {
      method: 'PUT',
      headers: auth.getAuthHeaders(),
      body: payload
    })
    ...
    const err = e as { data?: { error?: { message?: string } } }
    error.value = err?.data?.error?.message || 'Failed to update politician'
  }

  saving.value = false
}

onMounted(loadPolitician)

useSeoMeta({
  title: 'Edit Politician - Pulpulitiko Admin'
})
</script>
  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
      <div class="flex items-center gap-4">
        ...
        />
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Edit Politician</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update politician profile</p>
        </div>
      </div>
      <div class="sm:hidden">
        <UButton type="submit" form="politician-form" :loading="saving" class="w-full" size="lg">
          Save Changes
        </UButton>
      </div>
    </div>
    ...
    <div v-if="loading" class="py-16 text-center">
      ...
      <p class="text-gray-500 dark:text-gray-400">Loading politician...</p>
    </div>
