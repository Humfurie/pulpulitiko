...
async function loadUser() {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse<Author>>(`${baseUrl}/admin/users/${userId}`, {
      headers: auth.getAuthHeaders()
    })
...
useSeoMeta({
  title: 'Edit User - Pulpulitiko Admin'
})
</script>
...
<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update user account details</p>
        </div>
      </div>
      <div class="sm:hidden">
        <UButton type="submit" form="user-form" :loading="saving" class="w-full" size="lg">
          Update User
        </UButton>
      </div>
    </div>
    ...
    <div v-if="loading" class="py-16 text-center">
      ...
      <p class="text-gray-500 dark:text-gray-400">Loading user...</p>
    </div>
